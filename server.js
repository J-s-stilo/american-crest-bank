...
function makeTransferReceiptPdf({status,request,originalAmount,originalCurrency,convertedAmount,convertedCurrency}) {
  const successful = ['successful','approved','completed','success'].includes(String(status || '').toLowerCase());
  const label = successful ? 'SUCCESSFUL' : 'PENDING';
  const amountText = formatMoneyValue(originalAmount, originalCurrency);
  const convertedText = convertedAmount != null ? formatMoneyValue(convertedAmount, convertedCurrency || originalCurrency) : 'â€”';
  const dateText = new Date(request.created_at || Date.now()).toLocaleString();
  const customerNumber = request.customer_number || request.customerNumber || request.customer_id || request.user_id || 'â€”';
  const note = String(request.note || '').trim() || 'No note provided.';
  const senderName = request.sender_name || request.name || 'Customer';
  const senderEmail = request.sender_email || request.email || 'â€”';
  const senderPhone = request.sender_phone || request.phone || 'â€”';
  const recipient = request.recipient || 'Recipient';
  const recipientEmail = request.recipient_email || request.recipientEmail || 'â€”';
  const recipientPhone = request.recipient_phone || request.recipientPhone || 'â€”';
  const bank = request.recipient_bank || request.recipientBank || request.bank_name || request.bankName || 'External Bank';
  const account = request.recipient_account || request.recipientAccount || request.account_number || request.accountNumber || 'â€”';
  const swift = request.swift_bic || request.swiftBic || request.swift || 'â€”';
  const reference = request.id || 'N/A';
...
async function updateTransferStatus(req, res) {
  ...
  try {
    ...
    const debitCurrency =
      convertedAmount != null
        ? primaryCurrency
        : String(request.currency).toUpperCase();

    // Approval is the point at which the customer's balance is actually debited.
    // Keep the debit inside the same transaction as the request status change so
    // a failed/insufficient balance can never produce a successful transfer.
    const debitResult =
      await client.query(
        `
        UPDATE acb_balances
    ...
    if (!debitResult.rowCount) {
      ...
      return res.status(400).json({
        ok: false,
        error: `Insufficient ${debitCurrency} balance to approve this transfer.`
      });
    }
    ...
    await client.query(
      ...
      [
        ...
        `Your ONLINE BANKING transfer of ${Number(request.amount).toLocaleString()} ${request.currency} to ${request.recipient || 'the recipient'} was approved successfully. Your account balance has been debited.`
