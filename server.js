...
CUSTOMER NOTIFICATION BOARD — ADDITION ONLY

This block is isolated from the existing image/profile,
customer-support, login, verification and request handlers.
...
app.post('/api/notifications/:id/reply', auth, writeLimiter, async (req, res) => {
  ...
  try {
    ...
    if (admin.rowCount) {
      ...
      await pool.query(
        ...
        [
          ...
          `Customer ${customerName} replied to a notification: ${message}`
    ...
    console.error('Customer notification reply error:', error);

    return res.status(500).json({

      ok: false,

      error: 'Unable to send notification reply.'
