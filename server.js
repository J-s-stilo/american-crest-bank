...
function makeTransferReceiptPdf({status,request,originalAmount,originalCurrency,convertedAmount,convertedCurrency}) {
  ...
  const account = request.recipient_account || request.recipientAccount || request.account_number || request.accountNumber || '—';
  const swift = request.swift_bic || request.swiftBic || request.swift || '—';
