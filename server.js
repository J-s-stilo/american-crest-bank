...
async function getUser(userId) {
  ...
  /*
  Repair missing currency rows automatically.
  This is important for customers created by an older
...
The important change is that the resolver also checks
the database directly by email/name rather than relying
only on the value generated by the frontend.
=========================================================
*/
...
async function initDb() {
  ...
  /*
  Repair existing users created by older versions.
  */
  const existingUsers =
    await pool.query(`
      SELECT id
    ...
    */
    await client.query(
      ...
      [
        uuid(),
        userId,
        'Your American Crest demo account was created successfully.'
      ]
    );
...
These allow older versions of the frontend to continue
working if they use /api/register or /api/signup.
*/
app.post(
  '/api/register',
  authLimiter,
...
async function loginHandler(req, res) {
  try {
    ...
    /*
    Repair currency accounts for older customers.
    */
    await ensureBalances(
      databaseUser.id
    );
...
async function creditCustomerAccount({
  ...
  try {
    ...
    /*
    Make sure the selected currency account exists.
    */
    await client.query(
      `
      INSERT INTO acb_balances
...
async function handleAdminCredit(
  ...
  try {
    /*
    FIRST try the route ID.
    If it does not resolve, try every possible
    customer identifier supplied by the frontend.
    */
    ...
    */
    ...
    if (
      ...
      return res.status(400).json({
        ...
        error:
          'Enter a valid currency and amount.'
      });
    }
